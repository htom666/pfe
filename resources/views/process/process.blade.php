            ...
            <form id="fileupload-2" class="mt-3" action="{{route('process.upload')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row fileupload-buttonbar">
                    <div class="col-lg-7">
                        ...
                        <span class="btn btn-warning-lightened fileinput-button">
                            <span>Add Invoice</span>
                            <input type="file" name="invoice"/>
                        </span>
                        <button type="submit" class="btn btn-primary-lightened">
...
<div class="panel panel-light">
    <div class="panel-header">
        <h1 class="panel-title">Extract Invoice</h1>
    </div>
    <div class="panel-body">
...
<div class="panel panel-light">
    <div class="panel-header">
        <h1 class="panel-title">Extracted Invoice List</h1>
    </div>
    <div class="panel-body">
                ...
                <table id="datatable-column-search" class="table table-bordered">
                    ...
                    <tbody>
                            ...
                            <td>
                                ...
                                <img id="myImg" width="300" data-toggle="modal" data-target="#privacy-modal" height="200" src="{{asset('storage/invoices/' . $invoice->invoice)}}">
                                {{-- {{dd(asset('storage/app/public/invoices/' .$invoice->id))}} --}}
                            </td>
                            ...
                            <td class="operations">
                                 {{-- <a href="{{ route('product.show',$product->id)}}" class="btn-icon btn btn-primary-light" data-toggle="tooltip" title="View">
                                        ...
                                        <a href="{{ route('process.edit',$invoice->id)}}" class="btn-icon btn btn-primary-light" data-toggle="tooltip" title="Edit">
                                        ...
                                        @can('delete-extracted')
                                        <button type="button" class="delete-btns btn-icon btn btn-danger-light" data-toggle="modal" data-target="#confirm-modal">
...
<div class="modal fade" tabindex="-1" role="dialog" id="privacy-modal">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-privacy" role="document">
        <div class="center">
            @if($invoice->id ?? "" != null)
        {{-- <img src="{{asset('storage/invoices/' . $invoice->id . '/' . $invoice->invoice)}}" width="150%" height="150%" alt=""> --}}
        <img src="{{asset('storage/invoices/' . $invoice->invoice)}}" width="150%" height="150%" alt="">
        @endif
    </div>
    </div>
</div>
<div class="modal fade" tabindex="-1" role="dialog" id="confirm-modal">
    ...
    <div class="modal-dialog modal-dialog-centered modal-confirm confirm-danger">
        <form action="{{route('process.destroy',$invoice->id ?? "")}}" method="POST">
            @csrf
            @method('DELETE')
